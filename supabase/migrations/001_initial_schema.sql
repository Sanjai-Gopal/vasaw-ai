-- VASAW AI — INITIAL DATABASE SCHEMA (CORRECTED)
...
DROP POLICY IF EXISTS "Allow full access to campaigns via service role" ON campaigns;
CREATE POLICY "Allow full access to campaigns via service role"
...
CREATE TABLE IF NOT EXISTS leads (
  ...
  qualification_json JSONB DEFAULT '{"hasWebsite":false,"websiteQuality":0,"hasWhatsApp":false,"hasReviews":false,"responseLikelihood":"unknown","notes":""}',
...
DROP POLICY IF EXISTS "Allow full access to leads via service role" ON leads;
CREATE POLICY "Allow full access to leads via service_role" ON leads
...
DROP POLICY IF EXISTS "Allow read access to agent_runs" ON agent_runs;
CREATE POLICY "Allow read access to agent_runs"
ON agent_runs FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to agent_runs via service role" ON agent_runs;
CREATE POLICY "Allow full access to agent_runs via service_role" ON agent_runs
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow read access to websites" ON websites;
CREATE POLICY "Allow read access to websites"
ON websites FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to websites via service role" ON websites;
CREATE POLICY "Allow full access to websites via service_role" ON websites
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow read access to deployments" ON deployments;
CREATE POLICY "Allow read access to deployments"
ON deployments FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to deployments via service role" ON deployments;
CREATE POLICY "Allow full access to deployments via service_role" ON deployments
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow full access to messages via service role" ON messages;
CREATE POLICY "Allow full access to messages via service_role" ON messages
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow read access to conversations" ON conversations;
CREATE POLICY "Allow read access to conversations"
ON conversations FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to conversations via service role" ON conversations;
CREATE POLICY "Allow full access to conversations via service_role" ON conversations
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY "Allow read access to automations"
ON automations FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to automations via service role" ON automations;
CREATE POLICY "Allow full access to automations via service_role" ON automations
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY "Allow read access to automation_runs"
ON automation_runs FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to automation_runs via service role" ON automation_runs;
CREATE POLICY "Allow full access to automation_runs via service_role" ON automation_runs
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow full access to activities via service role" ON activities;
CREATE POLICY "Allow full access to activities via service_role" ON activities
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow full access to jobs via service role" ON jobs;
CREATE POLICY "Allow full access to jobs via service_role" ON jobs
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "Allow read access to provider_attempts" ON provider_attempts;
CREATE POLICY "Allow read access to provider_attempts"
ON provider_attempts FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow full access to provider_attempts via service role" ON provider_attempts;
CREATE POLICY "Allow full access to provider_attempts via service_role" ON provider_attempts
FOR ALL TO service_role USING (true) WITH CHECK (true);
...
GROUP BY
  c.id, c.name, c.status, c.lead_target,
  c.leads_collected, c.leads_qualified,
  c.websites_built, c.websites_deployed;
...
SELECT
  ...
  l.outreach_status,
  w.live_url,
  w.status AS website_record_status,
  w.build_progress,
